# Setting up ExternalDNS for Services on Cloudflare Tunnel

This tutorial describes how to setup ExternalDNS for usage within a Kubernetes cluster using Cloudflare Tunnel.
...
[Create a Cloudflare tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/)
...
When using API Token authentication, the token should be granted Zone `Read`, DNS `Edit` privileges, Cloudflare Tunnel `Edit` privileges, and access to `All zones`.
...
## Setting TunnelID and AccountID

Set then environment variable, `CF_TUNNEL_ID` and `CF_ACCOUNT_ID`.
      ...
      containers:
        - name: external-dns
          ...
          env:
            ...
            - name: CF_TUNNEL_ID
              value: "YOUR_CLOUDFLARE_TUNNEL_ID"
            - name: CF_ACCOUNT_ID
